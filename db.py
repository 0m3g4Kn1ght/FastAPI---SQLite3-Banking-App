...
def create_account(name: str) -> int:
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute("INSERT INTO accounts (name, balance) VALUES (?, ?)", (name, 0.0))
    conn.commit()
...
def get_account(account_id: int):
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute("SELECT id, name, balance FROM accounts WHERE id=?", (account_id,))
    row = c.fetchone()
    conn.close()
    return row
...
def search_accounts_by_name(name: str):
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute("SELECT id, name, balance FROM accounts WHERE name LIKE ?", (f"%{name}%",))
    rows = c.fetchall()
    conn.close()
    return rows
